...
知识图谱构建系统采用多阶段流水线架构。当前代码的主流程入口为：

- `knowledge_graph/__main__.py`（CLI 参数解析）
- `knowledge_graph/pipeline.py`（`run_full_pipeline()`：执行顺序与循环控制）

当前实现除 8 个核心步骤外，还包含：

- **Step 6.5：重聚合（recluster）**
- **Step 7.5：微调（refinement）**
...
- **知识点列表**：来自 EntityExtractorAgent（`state.knowledge_points`）

#### 处理流程

1. 初始化向量数据库（当前默认 LanceDB，见 `knowledge_graph/utils/vector_db.py`）
2. 批量向量化知识点并写入向量库
3. 写入 `state.vector_db_updated = True`
...
- **Stage1实体（L1）**：来自 L1ExtractorAgent / L1ValidatorAgent（最终以 `validated_l1_concepts` 为准）
...
3. **层级归属**：
   ...
   - L3知识点标记层级
4. 关系整合：合并前置关系与抽取关系
5. 关系验证与校准：`validate_and_calibrate_relationships()` 强约束 `contains` 只能是 `L1->L2` / `L2->L3` / `L3->L4`，并清理重复、多父、反向同类边
6. 保存校准结果：当前以 **Parquet** 为主
...
calibrated_resources: []   # 当前实现资源不进入 calibrated（资源仍保留在 stage3_resources）
...
- **校准后的知识点**：来自 CalibrationAgent/重聚合/微调后的最新图（`state.calibrated_kps`）
- **校准后的关系**：同上（`state.calibrated_relationships`）

> 评测阶段会过滤 `has_resource`，资源不参与评测（见 `knowledge_graph/agents/evaluation.py`）。
...
- **校准后的知识点**：来自校准/重聚合/微调后的最新图（`state.calibrated_kps`）
- **校准后的关系**：同上（`state.calibrated_relationships`）

> 当前实现会过滤 `has_resource`，只入库知识点图（见 `knowledge_graph/agents/graph_builder.py`）。
...
6. 创建关系：`contains` / `prerequisite` 等（过滤 `has_resource`）
8. **关闭连接**：释放数据库连接
...
**节点类型**：
- `KnowledgePoint`：知识点（支持 L1/L2/L3/L4）
  - id, name, definition, level
  - L2 额外写 `parent_id`
...
（当前默认不写入 `has_resource`）

---

### 2.9 GraphReclusterAgent - 重聚合智能体（Step 6.5）

| 属性 | 值 |
|------|-----|
| **Node编号** | Node 6.5 |
| **文件位置** | `knowledge_graph/agents/recluster.py` |
| **类名** | `GraphReclusterAgent` |
| **父类** | `LLMAgent` |

#### 功能说明

在校准后对每个 L1 簇执行重聚合，以满足“每个 L1 下 L2 数量上限”等结构性约束：

- old L2 下沉为 L3、old L3 下沉为 L4
- 新建聚合 L2（默认每个 L1 不超过 10 个）
- 重写 contains 关系：`L1->newL2`、`newL2->oldL2(now L3)`
- 下沉后会进行 **id 前缀规范化**（将下沉节点统一为 `l3_` / `l4_`，避免出现“level=L3 但 id 仍是 l2_ 前缀”）
...
### 2.10 GraphRefinementAgent - 微调智能体（Step 7.5）

| 属性 | 值 |
|------|-----|
| **Node编号** | Node 7.5 |
| **文件位置** | `knowledge_graph/agents/refinement.py` |
| **类名** | `GraphRefinementAgent` |
| **父类** | `LLMAgent` |

#### 功能说明

基于步骤7输出的 `evaluation_report.adjustment_suggestions` 执行结构微调，当前支持：

- **MERGE**：合并重复/同义节点（重定向关系并删除被合并节点）
- **ADD**：补充缺口知识点（既可能补 `L1->L2`，也可能补 `L2->L3`，受上限约束）
- **DELETE**：删除孤立节点（支持单点删除或批量删孤立）

动作执行顺序（以代码为准）：**MERGE → ADD → DELETE**（见 `_ACTION_ORDER`）。

#### 关键约束（高频踩坑）

- ADD 新增节点后会做关系校验，并可能因“新增节点出现孤立”等原因回滚动作
- L1 下 L2 数量存在上限（10）；L2 下 L3 上限/单次新增量由 `refinement.*` 配置控制
...
|-------|------------|------------|----------|
| L1Extractor | `l1_extractor.py` | `L1_Extraction_Prompt.txt` | `data/output/stage1_entities.parquet` |
| L1Validator | `l1_validator.py` | `L1_Validation_Prompt.txt` | `data/output/stage1_entities.parquet`（覆盖为验证版） |
| L1Prerequisite | `l1_prerequisite.py` | `L1_Prerequisite_Prompt.txt` | `data/output/stage2_relationships.parquet` |
| EntityExtractor | `entity_extractor.py` | `Entity_Extraction_Prompt.txt` | `data/output/stage3_*.parquet` |
| Vectorization | `vectorization.py` | - | 向量数据库（LanceDB） |
| Calibration | `calibration.py` | `Merge_Entities_Prompt.txt` | `data/output/calibrated_*.parquet` |
| Recluster | `recluster.py` | `Recluster_Prompt.txt` | 覆盖更新 `data/output/calibrated_*.parquet` |
| Evaluation | `evaluation.py` | `Evaluation_Prompt.txt` / `Cluster_Evaluation_Prompt.txt` | `data/output/evaluation/<run_id>/*` + `data/output/final_evaluation/**` |
| Refinement | `refinement.py` | `Refinement_Plan_Prompt.txt`（以及内部构造 ADD 提示） | 覆盖更新 `data/output/calibrated_*.parquet` |
| GraphBuilder | `graph_builder.py` | - | Neo4j 图（仅知识点图；过滤 has_resource） |
